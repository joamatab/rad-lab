...
def main(PR):
            ...
            for file in modified_files:
                ...
                if dir in file:
                    for raw in modified_files_raw:
                        ...
                        if file in raw:
                            print(f"Downloading file: {raw}")
                            downloadprfiles(raw, file, f'{os.getcwd()}/temp/{os.path.dirname(file)}')
                            break
    ...
    try:
        for dir in working_directories:
            ...
            comment, status = tf(f'{os.getcwd()}/temp/{dir}')
            comment = f'{comment} for: **{dir}** !'

            # Commenting on the PR
            commentpr(GITHUB_REPOSITORY, PR, comment, TOKEN)
            if (status == 'fail'):
                sys.exit(f'Terraform Init or Terraform Plan FAILED for: {dir}')
